...
def get_tencent_data():
    ...
    # 根据日期的确诊统计和新增确诊的数量
    dayListnew_url = "https://api.inews.qq.com/newsqa/v1/query/inner/publish/modules/list?modules=chinaDayListNew,chinaDayAddListNew&limit={}".format(
        limit_num)
    ...
    for i in data_daylist['chinaDayListNew']:
        ...
        if not ds:
            continue
        history[ds] = {'confirm': i['confirm'], 'heal': i['heal'], 'dead': i['dead'],
                       "confirm_now": i['nowConfirm'], 'importedCase': i['importedCase']}
    ...
    for i in data_daylist['chinaDayAddListNew']:
        ...
        history[ds].update({'confirm_add': i['confirm'], 'heal_add': i['heal'], 'dead_add': i['dead'],
    ...
    for pro_info in province_at:  # 拿到各个省分的详细数据
        print(pro_info['name'])
        province = pro_info['name']  # 省份名称
        now_confirm = pro_info['total']['nowConfirm']  # 获取当天的现有确诊数据
        ds_str = pro_info['date'].replace("/", '-')  # 更新的日期
        ex_pro_total = pro_info['total']
        ex_pro_today = pro_info['today']

        # 对当日省份的数据进行整合
        ex_pro_info = [ds_str, province, ex_pro_total['confirm'],
                       ex_pro_today['confirm'], now_confirm, ex_pro_total['heal'],
                       ex_pro_total['dead'], ex_pro_today['dead_add']]
        insert_to_detail(ex_pro_info)

        # 当日往前三个月的历史数据
        if pro_info['adcode'] == '':  # 港澳台没有历史数据
            continue
        ...
        for pro_day in pro_date_detail:
            ...
            pro_date = [ds, pro_day['province'], pro_day['confirm'],
                        pro_day['confirm_add'], None, pro_day['heal'], pro_day['dead'], pro_day['deadAdd']]
            # print(pro_date)
            # TODO 数量写入数据库中
            # 插入到数据库中
            insert_to_detail(pro_date)
        print(f'[{province}] 三个月份的数据写入成功，等待完成下一个省份的数据爬取.....')
        time.sleep(3)  # 爬虫的QPS最好设置为10，否则容易封ip，当然视不同网站而定

def load_signature():
    '''获取网站的加密参数'''
    e = str(int(time.time()))  # 时间戳
...
def get_risk_data():
    '''获取中高风险地区的内容'''
    x_wif_sign, qu_sign, ts = load_signature()  # 获取加密的值
    ...
    if resp.status_code == 200:
        ...
        utime = utime_date.strftime("%Y-%m-%d")
        # 中高风险地区的数量
...
def insert_to_history(data):
    '''全国每天的历史数据'''
    conn, cursor = get_conn()  # 获取数据库的连接和游标
    print(f"{time.asctime()}:开始更新历史数据...")
    try:
        sql = "insert into history values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        sql_query = "select confirm from history where ds=%s"
        for k, v in data.items():
            if not cursor.execute(sql_query, k):  # 如果这个日期存在，那么就不执行写入操作
        ...
        conn.rollback()  # 异常就进行回滚
        traceback.print_exc()  # 打印错误的信息
...
def insert_to_detail(data):
    '''每个省份每天的数据'''
    conn, cursor = get_conn()  # 获取数据库连接的引擎对象和游标
    try:
        sql = "insert into details(update_time,province,confirm,confirm_add,confirm_now,heal,dead,dead_add)" \
              "values(%s,%s,%s,%s,%s,%s,%s,%s)"
        sql_query = "select confirm from details where province=%s and update_time=%s"
        if not cursor.execute(sql_query, [data[1], data[0]]):  # 利用省份和日期进行检索
